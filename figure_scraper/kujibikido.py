...
class Kujibikido(Website):
    base_folder = constants.FOLDER_KUJIBIKIDO
    title = constants.WEBSITE_TITLE_KUJIBIKIDO
    keywords = ["https://kujibikido.com"]

    product_url_prefix = 'https://kujibikido.com/'
    ...
    def process_product_page(cls, page):
        ...
        try:
            ...
            images = soup.select('.kvArea .pcOnly img, .kvArea .kvItem img, .itemLineup .prizeList img, .specialimage img')
            if len(images) == 0:
