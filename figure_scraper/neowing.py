...
class Neowing(Website):
    ...
    def run(cls):
        ...
        while True:
            ...
            if evaluate:
                ...
                if is_scan:
                    scan_output_file = constants.FILE_NEOWING_SCAN_OUTPUT % today
                    print('The result of the scan is saved at: %s' % scan_output_file)
                    temp_folder = cls.base_folder + '/' + constants.SUBFOLDER_NEOWING_TEMP
    ...
    def scan_product_page(cls, product_id):
        id_ = str(product_id)
        product_url = cls.product_url_template % id_
        if not os.path.exists(constants.SUBFOLDER_NEOWING_SCAN):
            os.makedirs(constants.SUBFOLDER_NEOWING_SCAN)
        try:
            soup = cls.get_soup(product_url)
            ...
            if len(title) == 0 or len(jan) == 0:
                ...
                return
            filepath = cls.base_folder + '/' + constants.SUBFOLDER_NEOWING_TEMP + '/' + str(product_id)
