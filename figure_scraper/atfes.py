...
class Atfes(Website):
    base_folder = constants.FOLDER_ATFES
    title = constants.WEBSITE_TITLE_ATFES
    max_processes = 1
    keywords = ["https://shopping.atfes.com/"]

    product_url_template = 'https://shopping.atfes.com/product/%s'
    ...
    def process_by_product_id(cls):
        print('[INFO] Product Page URL is in the format: https://shopping.atfes.com/product/{id}')
        ...
        if is_scan:
            scan_output_file = constants.FILE_ATFES_SCAN_OUTPUT % today
            print('The result of the scan is saved at: %s' % scan_output_file)
            temp_folder = cls.base_folder + '/' + constants.SUBFOLDER_ATFES_TEMP
    ...
    def process_product_page(cls, product_id, folder=None, use_jan=False):
        ...
        try:
            ...
            images = soup.select('#js-products-image img[src]')
            if use_jan:
                product_json = cls.get_product_json(soup)
                jan_code = cls.get_jan_code(product_json)
            image_urls = set()
            for image in images:
    ...
    def scan_product_page(cls, product_id):
        id_ = str(product_id)
        product_url = cls.product_url_template % id_
        if not os.path.exists(constants.SUBFOLDER_ATFES_SCAN):
            os.makedirs(constants.SUBFOLDER_ATFES_SCAN)
        try:
            soup = cls.get_soup(product_url, verify=False)
            product_json = cls.get_product_json(soup)
            title = cls.get_title(product_json)
            jan = cls.get_jan_code(product_json)
            if len(title) == 0 or len(jan) == 0:
                print('[ERROR] Product ID %s does not exists.' % str(id_))
                return
            filepath = cls.base_folder + '/' + constants.SUBFOLDER_ATFES_TEMP + '/' + str(product_id)
    ...
    @staticmethod
    def get_jan_code(product_json):
        if product_json is None or 'description' not in product_json:
            return ''
        split_list = product_json['description'].split('\r\n')
        for split_ in split_list:
            s = split_.strip()
            if len(s) == 13 and s.isnumeric() and (s.startswith('45') or s.startswith('49')):
                return s
        return ''

    @staticmethod
    def get_title(product_json):
        if product_json is None or 'name' not in product_json:
            return ''
    ...
    def get_product_json(soup):
        try:
            ...
            if len(script_tag) == 0:
                return
            return json.loads(script_tag[0].string)
        except:
            return None
